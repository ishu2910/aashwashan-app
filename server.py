...
def send_email(name, email, phone, message):
    ...
    data = {
        "sender": {"email": "care@aashwashan.com"},
        "to": [{"email": "care@aashwashan.com"}],
...
async def create_appointment(
    ...
    try:
      ...
      #  await mongo_db.appointments.insert_one(doc)
        ...
        booking = Booking(
        therapist_id="408eb081-bfff-4f1c-83a2-969fcead9eb9",
    ...
    try:
        ...
        if request.message_type == "welcome":
            ...
            result = whatsapp_service.send_booking_confirmation(
                request.to_phone,
                request.user_name,
                request.therapist_name or "Our Therapist",
                request.date or "TBD",
                request.time or "TBD",
                request.duration or "45 minutes",
            ...
            result = whatsapp_service.send_afternoon_checkin(
                request.to_phone,
                request.user_name
            )

        elif request.message_type == "evening":
            result = whatsapp_service.send_evening_reflection(
                request.to_phone,
                request.user_name
            )
...
async def book_session(data: BookingRequest):
